...
public class CRBM {
	...
	public CRBM(int numberVisible, int numberHidden,
			double learning_rate_weights, double learning_rate_aj,
			double theta_low, double theta_high, double sigma) {
		...
		// Randomly inicializes the weight matrix, whose size is (numberVisible
		// x numberHidden).
	...
	private double[][] calculateSigmoid(double[][] activations,
			double[][] aj_matrix) {
		...
		for (int i = 0; i < activations.length; i++) {
			for (int j = 0; j < activations[0].length; j++) {
				sigmoid_matrix[i][j] = theta_low + (theta_high - theta_low)
						* logisticFunction(aj_matrix[i][j] * activations[i][j]);
			}
		}
	...
	public double[][] train(double[][] training_set, int number_epochs) {
		...
		// Local matrices of 'noise controls' (aj). Every neuron has its own
		// noise control.
		...
		for (int epoch = 1; epoch <= number_epochs; epoch++) {
			...
			double[][] pos_hidden_sj = calculateSigmoid(pos_hidden_activations,
					_aj_hidden);
			// Corresponds to <si*sj>, used to update the weights.
			double[][] pos_associations = dot(getTranspose(training_set),
					pos_hidden_sj);
			...
			double[][] neg_visible_activations = dot(pos_hidden_sj,
					getTranspose(weights));
			neg_visible_sj = calculateSigmoid(neg_visible_activations,
					_aj_visible);
			...
			double[][] neg_associations = dot(getTranspose(neg_visible_sj),
					neg_hidden_sj);
			...
			for (int i = 0; i < weights.length; i++) {
				for (int j = 0; j < weights[0].length; j++) {
					weights[i][j] += learning_rate_weights
							* ((pos_associations[i][j] - neg_associations[i][j]) / (double) number_training_examples);
				}
			}
			...
			for (int current_example = 0; current_example < number_training_examples; current_example++) {
				...
				for (int visible_neuron_iterator = 0; visible_neuron_iterator < _aj_visible[0].length; visible_neuron_iterator++) {
					_aj_visible[current_example][visible_neuron_iterator] += (learning_rate_aj / Math
							.pow(_aj_visible[current_example][visible_neuron_iterator],
									2))
							* (((Math
									.pow(training_set[current_example][visible_neuron_iterator],
											2)) - (Math
									.pow(neg_visible_sj[current_example][visible_neuron_iterator],
				...
				for (int hidden_neuron_iterator = 0; hidden_neuron_iterator < _aj_hidden[0].length; hidden_neuron_iterator++) {
					_aj_hidden[current_example][hidden_neuron_iterator] += (learning_rate_aj / Math
							.pow(_aj_hidden[current_example][hidden_neuron_iterator],
									2))
							* (((Math
									.pow(pos_hidden_sj[current_example][hidden_neuron_iterator],
											2)) - (Math
									.pow(neg_hidden_sj[current_example][hidden_neuron_iterator],
											2))) / (double) number_training_examples);
				}
			}
			...
			if (epoch == number_epochs) {
				double error = CRBM.meanSquareError(training_set,
						neg_visible_sj);
		...
		// Global hidden layer is now the hidden layer states after the negative
		// phase.
	...
	public static double meanSquareError(double[][] training_set,
			double[][] reconstruction) {
		double error = 0;

		for (int i = 0; i < training_set.length; i++) {
			for (int j = 0; j < reconstruction[0].length; j++) {
				error += Math.pow((training_set[i][j] - reconstruction[i][j]),
						2);
			}
		}
	...
	private double[][] multiplyMatrices(double a[][], double b[][]) {
		...
		// System.out.println("a[" + a.length + "][" + a[0].length + "]");
		// System.out.println("b[" + b.length + "][" + b[0].length + "]");
		// System.out.println("resultant[" + resultant.length + "][" +
		// resultant[0].length + "]");
	...
	public static void main(String[] args) {
		...
		// Parameters with the same values used in "Continuous restricted
		// Boltzmann
		// machine with an implementable training algorithm", by H. Chen e A.F.
		// Murray.
