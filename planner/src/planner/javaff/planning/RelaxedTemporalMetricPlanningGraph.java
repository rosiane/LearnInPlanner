...
import planner.javaff.data.temporal.DurationFunction;
...
public class RelaxedTemporalMetricPlanningGraph extends
	...
	protected void resetAll(State S) {
		...
		if (S instanceof TemporalMetricState) {
			...
			addEndActionGoals((TemporalMetricStateDelta) S);
		}
	}
	...
	protected void addEndActionGoals(TemporalMetricStateDelta S) {
		Iterator dait = S.openActions.iterator();
		while (dait.hasNext()) {
			DurativeAction da = (DurativeAction) dait.next();
			Proposition p = da.dummyGoal;
	...
	public Plan getPlan(State s)// if on the start action is in also add the end
