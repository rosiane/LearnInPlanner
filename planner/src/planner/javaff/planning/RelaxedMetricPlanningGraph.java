...
import planner.javaff.data.metric.BinaryFunction;
import planner.javaff.data.metric.Function;
import planner.javaff.data.metric.MetricSymbolStore;
import planner.javaff.data.metric.NamedFunction;
import planner.javaff.data.metric.NumberFunction;
import planner.javaff.data.metric.ResourceOperator;


public class RelaxedMetricPlanningGraph extends RelaxedPlanningGraph {
	protected Set<PGBinaryComparator> metricGoal = new HashSet<PGBinaryComparator>();

	protected List<Map<PGFunction, BigDecimal>> minResources = null; // List of
																		// Maps
																		// [(PGFunction
																		// =>
	// BigDecimal)]
	protected List<Map<PGFunction, BigDecimal>> maxResources = null;

	protected Map<NamedFunction, PGNamedFunction> PGFuncMap = new Hashtable<NamedFunction, PGNamedFunction>(); // (NamedFunction
																												// =>
	// PGNamedFunction)
	protected Map<PGAction, Set<PGBinaryComparator>> ActionComparators = new Hashtable<PGAction, Set<PGBinaryComparator>>(); // (PGAction
	// =>
	// (PGBinaryComparator))
	protected Map<PGAction, Set<PGResourceOperator>> ActionOperators = new Hashtable<PGAction, Set<PGResourceOperator>>(); // (PGAction
																															// =>
	// (PGFunctionOperator))

	// Used during graph construction
	private Set<PGResourceOperator> resOps = null; // PGResourceOperators that
													// are in the graph
	...
	private void setupPGFuncMap(Set<NamedFunction> funcs) {
		Iterator<NamedFunction> fit = funcs.iterator();
		NamedFunction nf = null;
		while (fit.hasNext()) {
			nf = fit.next();
	...
	private void makeComparators(Set<PGAction> actions) {
		Iterator<PGAction> ait = actions.iterator();
		PGAction pga = null;
		Set<PGBinaryComparator> ss = null;
		Set<BinaryComparator> cs = null;
		Iterator<BinaryComparator> cit = null;
		BinaryComparator bc = null;
		while (ait.hasNext()) {
			pga = ait.next();
			ss = new HashSet<PGBinaryComparator>();
			ActionComparators.put(pga, ss);
			cs = pga.getComparators();
			cit = cs.iterator();
	...
	private void makeOperators(Set<PGAction> actions) {
		Iterator<PGAction> ait = actions.iterator();
		PGAction pga = null;
		Set<PGResourceOperator> s = null;
		Iterator<ResourceOperator> oit = null;
		Set<ResourceOperator> os = null;
		ResourceOperator ro = null;
		while (ait.hasNext()) {
			pga = ait.next();
			s = new HashSet<PGResourceOperator>();
	...
	protected void setGoal(GroundCondition g) {
		super.setGoal(g);
		Iterator<BinaryComparator> cit = g.getComparators().iterator();
		BinaryComparator c = null;
		while (cit.hasNext()) {
			c = cit.next();
	...
	protected void resetAll(State s) {
		super.resetAll(s);
		maxResources = new ArrayList<Map<PGFunction, BigDecimal>>();
		minResources = new ArrayList<Map<PGFunction, BigDecimal>>();
		setInitialValues((MetricState) s);
	}

	protected void setInitialValues(MetricState ms) {
		Iterator<NamedFunction> fit = ms.funcValues.keySet().iterator();
		Map<PGFunction, BigDecimal> max = new Hashtable<PGFunction, BigDecimal>();
		Map<PGFunction, BigDecimal> min = new Hashtable<PGFunction, BigDecimal>();
		NamedFunction nf = null;
		PGFunction pgf = null;
		BigDecimal bd = null;
		while (fit.hasNext()) {
			nf = fit.next();
			pgf = PGFuncMap.get(nf);
			bd = ms.funcValues.get(nf);
			max.put(pgf, bd);
			min.put(pgf, bd);
		}
	...
	protected ArrayList<PGAction> createFactLayer(
			Set<PGProposition> scheduledFacts, int layer) {
		ArrayList<PGAction> rActionList = super.createFactLayer(scheduledFacts,
				layer);
	...
	private void updateResourceValues(int layer) {
		...
		Hashtable<PGFunction, BigDecimal> maxminus = (Hashtable<PGFunction, BigDecimal>) maxResources
				.get(layer - 1);
		Hashtable<PGFunction, BigDecimal> minminus = (Hashtable<PGFunction, BigDecimal>) minResources
				.get(layer - 1);
		@SuppressWarnings("unchecked")
		Map<PGFunction, BigDecimal> newmax = (Hashtable<PGFunction, BigDecimal>) maxminus
				.clone();
		@SuppressWarnings("unchecked")
		Map<PGFunction, BigDecimal> newmin = (Hashtable<PGFunction, BigDecimal>) minminus
				.clone();
		...
		Iterator<PGResourceOperator> roit = resOps.iterator();
		PGResourceOperator ro = null;
		BigDecimal max = null;
		BigDecimal min = null;
		BigDecimal nmax = null;
		BigDecimal nmin = null;
		while (roit.hasNext()) {
			ro = roit.next();
			max = ro.resource
					.getMaxValue(layer - 1, maxResources, minResources);
			min = ro.resource
					.getMinValue(layer - 1, maxResources, minResources);
			nmax = ro.maximise(layer - 1, maxResources, minResources);
			nmin = ro.minimise(layer - 1, maxResources, minResources);
	...
	protected HashSet calculateActionMutexesAndProps(Set filteredSet, int pLayer) {
		HashSet rSet = super
				.calculateActionMutexesAndProps(filteredSet, pLayer);
	...
	protected class PGResourceOperator {
		...
		public boolean increase(PGFunction f) {
			if (resource == f) {
				if (type == MetricSymbolStore.ASSIGN)
					return true; // could do something better here by looking at
									// the values
				else if (type == MetricSymbolStore.INCREASE)
					return true;
				else if (type == MetricSymbolStore.DECREASE)
