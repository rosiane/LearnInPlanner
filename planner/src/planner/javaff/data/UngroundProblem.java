...
import planner.javaff.data.metric.FunctionSymbol;
import planner.javaff.data.strips.Operator;
...
public class UngroundProblem {
	...
	public GroundProblem ground(final ParameterSearch parameterSearch) {
		...
		NeuralNetworkIF neuralNetwork = null;
		Layer[] net = null;
		Weight[] weights = null;
		if (parameterSearch != null) {
			features = parameterSearch.getFeatures();
			neuralNetwork = parameterSearch.getNeuralNetwork();
			net = parameterSearch.getNet();
			weights = parameterSearch.getWeights();
		}
