...
import planner.javaff.data.strips.EitherType;
import planner.javaff.data.strips.NOT;
import planner.javaff.data.strips.NullEffect;
...
import planner.javaff.data.strips.TrueCondition;
import planner.javaff.data.strips.UngroundInstantAction;
import planner.javaff.data.strips.Variable;
import planner.javaff.data.temporal.DurationConstraint;
import planner.javaff.data.temporal.SimpleDurationConstraint;
import planner.javaff.data.temporal.UngroundDurativeAction;
...
public class PDDL21parser implements PDDL21parserConstants {
    ...
    private static boolean parseDomainFile(File pFile)
    {
        ...
        try
        {
            ...
            tFileReader.close();
                ...
                {
            Planner.parsingOutput.println("File "+pFile+" has not been found");
            req = false;
                }
                catch (IOException e)
                {
            Planner.parsingOutput.println("Unknown IOException caught");
            Planner.parsingOutput.println(e.getMessage());
            req = false;
                }
        catch (ParseException e)
        {
            Planner.parsingOutput.println("Error whilst parsing file "+pFile);
            Planner.parsingOutput.println(e.getMessage());
            req = false;
        }
        if (req) Planner.parsingOutput.println("Parsed Domain file "+pFile+" successfully");
                else Planner.parsingOutput.println("Parsing of Domain file "+pFile+" failed");
                return req;
    }
    ...
    private static boolean parseProblemFile(File pFile)
    {
        ...
        if (probsuc) Planner.parsingOutput.println("Parsed Problem file "+pFile+" successfully");
                else Planner.parsingOutput.println("Parsing Problem file "+pFile+" failed");
                return probsuc;
    }
    ...
    while (true) {
      ...
      if (jj_2_8(20)) {
        ...
        t = jj_consume_token(NON_HANDLED_LABELS);
                                                                                                                       Planner.parsingOutput.println("cannnot parse "+t.toString()); {if (true) return false;}
      } else {
        jj_consume_token(-1);
  ...
  final public boolean parseProblem() throws ParseException {
    ...
    jj_consume_token(CLOSE);
                ...
                if (!UP.DomainName.toLowerCase().equals(t2.toString().toLowerCase()))
                        Planner.parsingOutput.println("Warning Domain Name and Problem Name do not match");
    if (jj_2_75(20)) {
      parseRequireDef();
    } else {
      ;
